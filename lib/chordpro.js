...
module.exports = {
  ...
  getSection: function(song, id) {
    ...
    return getSongSection(song, id);
  },

  moveChord: function(song, from, to) {

    for(ci = 0; ci < song.chords.length; ci++) {
      if (from.line == song.chords[ci].line && from.col == song.chords[ci].col && from.name == song.chords[ci].name) {
        song.chords[ci].col = to.col;
      }
    }
  ...
  copyChords: function(song, fromSectionId, toSectionId) {
    var newChords = [];

    for (i = 0; i < song.chords.length; i++) {
      if (song.chords[i].line < song.sections[toSectionId].start) {
        newChords.push(song.chords[i]);
      }
    }

    var section = getSongSection(song, fromSectionId);

    for (i = 0; i < section.chords.length; i++) {
      section.chords[i].line += (song.sections[toSectionId].start + 1);
      newChords.push(section.chords[i]);
    }

    for (i = 0; i < song.chords.length; i++) {
      if (song.chords[i].line > song.sections[toSectionId].end) {
        newChords.push(song.chords[i]);
      }
    }
...
function getSongSection(song, id) {
